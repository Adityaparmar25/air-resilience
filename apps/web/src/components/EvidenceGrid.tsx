...
export default function EvidenceGrid({
  ...
  const signalList: { key: keyof EvidenceBreakdown; signal: EvidenceSignal; icon: React.ReactNode }[] = [
    {
      key: "ground_anomaly",
      signal: evidence.ground_anomaly,
      icon: (
    ...
    {
      key: "citizen_reports",
      signal: evidence.citizen_reports,
      icon: (
    ...
    {
      key: "weather_context",
      signal: evidence.weather_context,
      icon: (
    ...
    {
      key: "fire_radiative_power",
      signal: evidence.fire_radiative_power,
      icon: (
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
        {signalList.map(({ key, signal, icon }) => {
          const isAvailable = signal.available && signal.score !== null;
            ...
            >
              ...
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs font-bold text-slate-200 truncate" title={signal.name}>
                    {signal.name}
                  </span>
                </div>
              ...
              <div className="mt-2 space-y-1">
                ...
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-mono font-bold text-slate-200">
                    {isAvailable ? signal.score?.toFixed(2) : "—"}
                  </span>
                </div>

                {signal.weighted_contribution !== undefined && signal.weighted_contribution !== null && (
              ...
              {/* Notes */}
              {signal.notes && (
                <p className="mt-2 text-[10px] text-slate-400 line-clamp-2 leading-relaxed bg-slate-950/60 p-1.5 rounded border border-slate-800/40">
                  {signal.notes}
                </p>
              )}
            </div>
